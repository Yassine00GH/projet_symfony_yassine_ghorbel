...
@section('content')
        ...
        <div class="relative overflow-hidden bg-gradient-to-br from-blue-600 to-indigo-700 rounded-2xl shadow-lg p-6 group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between relative z-10">
                <div>
                    ...
                    <p class="text-4xl font-extrabold mt-1">{{ number_format($stats['total_products']) }}</p>
                </div>
            ...
            <div class="mt-4 flex items-center text-xs text-blue-100">
                <span class="bg-blue-100/20 px-2 py-0.5 rounded-full">+{{ $stats['total_products'] > 0 ? 'Actif' : 'N/A' }}</span>
        ...
        <div class="relative overflow-hidden bg-gradient-to-br from-purple-600 to-pink-700 rounded-2xl shadow-lg p-6 group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between relative z-10">
                <div>
                    ...
                    <p class="text-4xl font-extrabold mt-1">{{ $stats['total_categories'] }}</p>
                </div>
                <div class="p-3 bg-white/20 backdrop-blur-md rounded-xl">
                    <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="relative overflow-hidden bg-gradient-to-br from-amber-500 to-orange-600 rounded-2xl shadow-lg p-6 group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between relative z-10">
                <div>
                    ...
                    <p class="text-4xl font-extrabold mt-1 text-white">{{ $stats['stock_faible'] }}</p>
                </div>
                <div class="p-3 bg-white/20 backdrop-blur-md rounded-xl">
                    <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="relative overflow-hidden bg-gradient-to-br from-rose-600 to-red-700 rounded-2xl shadow-lg p-6 group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
            ...
            <div class="flex items-center justify-between relative z-10">
                <div>
                    ...
                    <p class="text-4xl font-extrabold mt-1">{{ $stats['rupture_stock'] }}</p>
                </div>
                <div class="p-3 bg-white/20 backdrop-blur-md rounded-xl">
                    <svg class="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="lg:col-span-4 bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
            ...
            <div class="mt-4 grid grid-cols-3 gap-2 text-center text-xs">
                <div class="p-2 rounded-xl bg-green-50">
                    ...
                    <p class="font-bold text-green-600">{{ $stockStatusData['disponible'] }}</p>
                </div>
                <div class="p-2 rounded-xl bg-orange-50">
                    <p class="text-gray-500 mb-1">Warning</p>
                    <p class="font-bold text-orange-600">{{ $stockStatusData['alerte'] }}</p>
                </div>
                <div class="p-2 rounded-xl bg-red-50">
                    <p class="text-gray-500 mb-1">Rupture</p>
                    <p class="font-bold text-red-600">{{ $stockStatusData['rupture'] }}</p>
                </div>
            </div>
        </div>
    ...
    {{-- Tables (Original ones styled better) --}}
                        ...
                        @forelse($produitsAlerte as $p)
                            <tr class="hover:bg-gray-50/50 transition">
                                <td class="px-4 py-4">
                                    ...
                                    <div class="text-xs text-gray-400 font-medium">{{ $p->category->nom ?? '-' }}</div>
                                </td>
                                <td class="px-4 py-4">
                                    <div class="flex items-center">
                                        <span class="text-sm font-black text-gray-800">{{ $p->quantite }}</span>
                                        <span class="text-xs text-gray-400 mx-1">/</span>
                                        <span class="text-xs text-gray-400">{{ $p->seuil_alerte }}</span>
                                    </div>
                                    <div class="w-16 bg-gray-100 h-1 rounded-full mt-1">
                                        <div class="bg-orange-500 h-1 rounded-full" style="width: {{ min(100, ($p->quantite / max(1, $p->seuil_alerte)) * 100) }}%"></div>
                                    </div>
                                </td>
                                <td class="px-4 py-4">
                                    <a href="{{ route('admin.products.edit', $p) }}" class="text-blue-600 hover:text-blue-800 font-bold text-xs uppercase tracking-tighter">Réappro</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($produitsRecents as $p)
                            <tr class="hover:bg-gray-50/50 transition">
                                <td class="px-4 py-4 font-bold text-gray-800 text-sm">{{ $p->nom }}</td>
                                <td class="px-4 py-4 text-xs font-mono text-gray-400">{{ $p->reference }}</td>
                                <td class="px-4 py-4 text-right text-sm font-black text-emerald-600">{{ number_format($p->prix_vente, 0) }} DT</td>
                            </tr>
                        @empty
                            <tr>
...
<script>
    ...
    new ApexCharts(document.querySelector("#healthGauge"), {
        series: [{{ round(($stockStatusData['disponible'] / max(1, $stats['total_products'])) * 100) }}],
    ...
    new ApexCharts(document.querySelector("#financialChart"), {
        series: [{
            name: 'Coût Achat',
            data: [0, {{ $stats['valeur_stock'] * 0.4 }}, {{ $stats['valeur_stock'] * 0.7 }}, {{ $stats['valeur_stock'] }}]
        }, {
            name: 'Vente Potentielle',
            data: [0, {{ $stats['valeur_vente'] * 0.5 }}, {{ $stats['valeur_vente'] * 0.8 }}, {{ $stats['valeur_vente'] }}]
        }],
    ...
    new ApexCharts(document.querySelector("#topProductsChart"), {
        series: [{ data: {!! json_encode($topProduitsValeur->pluck('valeur_totale')) !!} }],
        chart: { type: 'bar', height: 300, toolbar: { show: false } },
        plotOptions: { bar: { borderRadius: 8, horizontal: true, distributed: true, barHeight: '60%' } },
        colors: chartColors,
        dataLabels: { enabled: false },
        xaxis: { categories: {!! json_encode($topProduitsValeur->pluck('nom')) !!}, labels: { show: false } },
        grid: { show: false }
    }).render();

    // 4. Catégories (Donut)
    new ApexCharts(document.querySelector("#categoriesChart"), {
        series: {!! json_encode($repartitionCategories->pluck('products_count')) !!},
        labels: {!! json_encode($repartitionCategories->pluck('nom')) !!},
