                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($users as $user)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-3 text-sm text-gray-600">{{ $user->telephone ?: '-' }}</td>
                            <td class="px-6 py-3">
                                @if($user->role === 'admin')
                            ...
                            <td class="px-6 py-3 text-right">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    @if($user->id !== auth()->id())
                                        <form method="POST" action="{{ route('admin.users.destroy', $user) }}" data-confirm="Supprimer cet utilisateur ?" class="inline">
                                            @csrf @method('DELETE')
